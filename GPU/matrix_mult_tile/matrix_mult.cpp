...
//--------------------------------------------------------------------
  const cl_int factor = 16;
  const cl_int sizeOfGroup = 4;
  const cl_int N = sizeOfGroup*sizeOfGroup*factor; // Number of rows
  const cl_int M = sizeOfGroup*sizeOfGroup*factor; // Number of columns
  ...
  cl_int errcode;
    ...
    const size_t work_group_size[2] =  {sizeOfGroup,sizeOfGroup};

    status = clEnqueueNDRangeKernel(queue, kernel, 2, NULL,
        global_work_size, work_group_size, 2, write_event, &kernel_event);
    checkError(status, "Failed to launch kernel");
    status = clEnqueueBarrier(queue);
    checkError(status, "Failed to create barrier");
